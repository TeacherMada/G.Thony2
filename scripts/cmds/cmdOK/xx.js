...
module.exports = {
  config: {
    ...
    countDown: 120,
    role: 2,
    shortDescription: "[👑] xx video 18+",
    longDescription: "get 18+ video",
    category: "𝟭𝟴+",
    guide: "{p}{n} on / off",
  },
  ...
  onStart: async function ({ api, event, args, message }) {
    ...
    if (command === "on") {
      if (isRandomVideoEnabled) {
        api.sendMessage("[on] xx is already enabled.", event.threadID, event.messageID);
      } else {
        isRandomVideoEnabled = true;
        api.sendMessage("[on] xx is now enabled", event.threadID, event.messageID);
        startSendingRandomVideos(api, event.threadID);
      }
    } else if (command === "off") {
      if (!isRandomVideoEnabled) {
        api.sendMessage("[off] xx is already disabled.", event.threadID, event.messageID);
      } else {
        isRandomVideoEnabled = false;
        clearInterval(intervalId);
        api.sendMessage("[off] xx is now disabled.", event.threadID, event.messageID);
      }
    } else {
      api.sendMessage("Choisissez '#xx on' pour commencer, ou '#xx off' to stop.", event.threadID, event.messageID);
    }
  },
};
